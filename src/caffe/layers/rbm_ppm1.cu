...
void RBMPPM1Layer<Dtype>::gradient_gpu(const vector<Blob<Dtype>*>& top,
...
*/
	...
	Dtype* X1S = this->X1Chain.mutable_gpu_data();
	Dtype* H1S = this->H1Chain.mutable_gpu_data();
...
*/
		...
		if(MLGASSERT<Dtype>::getInstance().mlg_gpu_finite(this->X1Chain.count(), this->X1Chain.gpu_data())) LOG(INFO) << "X1 not finite" << std::endl;
		if(MLGASSERT<Dtype>::getInstance().mlg_gpu_finite(this->H1Chain.count(), this->H1Chain.gpu_data())) LOG(INFO) << "H1 not finite" << std::endl;
		if(MLGASSERT<Dtype>::getInstance().mlg_gpu_range(this->X1Chain.count(), this->X1Chain.gpu_data())) LOG(INFO) << "X1 not in float range" << std::endl;
		if(MLGASSERT<Dtype>::getInstance().mlg_gpu_range(this->H1Chain.count(), this->H1Chain.gpu_data())) LOG(INFO) << "H1 not in float range" << std::endl;

	this->find_map_gpu(&(this->X1Chain), &(this->H1Chain), &bTmp, &cTmp, this->blobs_[0].get());
	...
	switch(this->layer_param_.rbm_param().rbm_pm_param().map_method()){
		case RBMPMLayer<Dtype>::CoordinateDescent:
		{
			X1S = this->X1Chain.mutable_gpu_data();
			H1S = this->H1Chain.mutable_gpu_data();
		}
		break;
		case RBMPMLayer<Dtype>::GreedyEnergyOptimization:
		{
...
template
void RBMPPM1Layer<float>::gradient_gpu(const vector<Blob<float>*>& top, \
      const vector<bool>& propagate_down, \
      const vector<Blob<float>*>& bottom);

template
void RBMPPM1Layer<double>::gradient_gpu(const vector<Blob<double>*>& top, \
      const vector<bool>& propagate_down, \
